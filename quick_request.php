 ...
 * 2. D/CEO OR CEO
 * 
 * BOTH USERS ARE HAVING PREVIALGES ACCORDING TO THEIR ROLES FROM SESSION
 */
            ...
            -->
                ...
                if (isset($_GET['operation-status']) || isset($_GET['approval']) || isset($_GET['reject'])) {
                    ...
                    if ($statement):
                        ...
                        /**
                         * APRROVING FUEL OPERATION REQUEST
                         */
                        ...
                        if (isset($_GET['approval'])) {
                            ...
                            if ($result->num_rows === 0) {
                                ...
                                if ($approve) {
                                    $success = "Operation Request Approved";
                                } else {
                                    $errors[] = "Operation not Approved, Try again";
                                }
                        ...
                        if (isset($_GET['reject'])) {
                            ...
                            if ($result->num_rows === 0) {
                                $errors[] = "reject ID not found";
                            } else {
                                // Reject status using prepared statement
                                $rejectStmt = $db->prepare("UPDATE operation SET status = 'rejected' WHERE id = ?");
                                $rejectStmt->bind_param("i", $rejectId);
                                $reject = $rejectStmt->execute();

                                if ($reject) {
                                    $success = "Operation Request Rejected";
                                } else {
                                    $errors[] = "Operation not Rejected, Try again";
                                }
